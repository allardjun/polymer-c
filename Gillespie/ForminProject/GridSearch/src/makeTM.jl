...
function makeTM(saveloc, construct_names, construct_PRM_locs, construct_PRM_sizes, c_PA::Float64, G::Float64, k_cap::Float64, k_del::Float64, r_cap::Float64, r_del::Float64, k_rel::Float64, r_cap_exp::Float64, prname, fname, savfname, saveTF, runGillespieTF, runtime=10000.0, timeavgval=5000.0)
    ...
    for k in 1:length(construct_PRM_sizes)
        cname=construct_names[k]
        println("starting construct $cname")
        flush(stdout)
        ...
        states, numValidStates, transitionMatrix, isBound, isDelivered = @btime generateTMtemplate($(PRM_locs))
        @info "make TM template"
        println("made TM template for $cname")
        flush(stdout)

        output_mat, output_cell = @btime readInMSBFiles($(fname), $(states), $(PRM_locs))
        @info "read in MSB files"
        println("read in MSB files for $cname")
        flush(stdout)

        # Get relevant polymer stats
        pocc = @btime getStateVals($(output_mat), $(PRM_locs), "POcclude")
        prvec = @btime getStateVals($(output_mat), $(PRM_locs), $(prname))
        pocc_0 = @btime getStateVals($(output_mat), [1], "POcclude")
        println("got polymer stats for $cname")
        flush(stdout)
        ...
        @info "made transition matrix successfully"
        println("made full TM for $cname")
        flush(stdout)
        ...
        if runGillespieTF
            ...
            @info "starting gillespie"
            println("about to runGillespie")
            flush(stdout)
            storedTotalTime_End, storedAddedActin_End, storedKpoly_End = @btime runGillespie($(transitionMatrix), $(states), $(numValidStates),$(2*length(PRM_locs)), $(runtime), $(output_file), $(timeavgval),$(saveTF))
            @info "ran gillespie"
            println("finished runGillespie")
            flush(stdout)
            kpolys[construct_names[k]] = [storedTotalTime_End, storedAddedActin_End, storedKpoly_End]
            println("stored gillespie info for $cname")
            flush(stdout)
        end
    end
    ...
    @info "saved kpolys.txt for $saveloc, $savfname"
    println("saved kpolys.txt for $saveloc, $savfname")
    flush(stdout)
