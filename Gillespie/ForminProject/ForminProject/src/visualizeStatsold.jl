...
# function visualizeStatsold(outDict::Dict{Vector{Vector{Int}},Dict{String, Any}}, stat::String)
...
function visualizeStatsold(outDict::Dict{Vector{Vector{Int}},Dict{String, Any}}, stat::String)
    ...
    # Store the sorted key order
    ordered_keys = dict_keys
    ...
    # Convert keys into string labels for y-axis
    y_tick_labels = [string(key) for key in ordered_keys]
    y_tick_positions = 1:length(ordered_keys)
    
    # Create the heatmap with updated y-axis labels
    Plots.plot()
    p = Plots.heatmap(
        1:nPRMs, y_tick_positions, data_matrix,
        xlabel="PRM Location", ylabel="Binding Site Keys",
        xticks=(1:nPRMs, PRMlocs),
        yticks=(y_tick_positions, y_tick_labels), 
        title="$stat Heatmap"
    )
