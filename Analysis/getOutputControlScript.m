% GETOUTPUTCONTROLSCRIPT Read a vertical ASCII file of output values generated by outputControl.c
% and convert to a Matlab struct object
% allardlab.com
% 
% Does not save the struct or workspace, but the variable output_struct is
% added to the workspace.
%
% Ser the input_file_path variable to the input ASCII file. The script will
% generate two additional files in the process: input_file_path.sorted and
% input_file_path.sorted.nosquare
%
%   Using the function getOutputControl is prefered.
% 
% See also GETOUTPUTCONTROL.

input_file_path = '../testoutputs/outputTest2copy.txt';
...
system(command);
command = sprintf( 'sed ''s/\\[[^]]*\\]//g'' %s.sorted > %s.sorted.nosquare', input_file_path, input_file_path);
system(command);
...
fileID = fopen(strcat(input_file_path,'.sorted.nosquare'), 'r');
if fileID == -1
