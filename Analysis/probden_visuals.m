function probden_visuals(n1,fh1length,FH2sizes,makeFH1sweeps,saveTF,savefigfolder)
% PROBDEN_VISUALS makes a series of plots to visualize the probability
% density equations
% 
% PROBDEN_VISUALS(n1,fh1length,FH2sizes,makeFH1sweeps,saveTF,savefigfolder)
% 
% Inputs:
%       n1           : (double) PRM location
%       fh1length    : (double) FH1 length
%       FH2sizes     : array of FH2 sizes to sweep (distance between tethers)
%       makeFH1sweeps: whether or not to make the Fh1 sweep plots (these
%                       only need to be made once, default is false)
...
%                       string)
% 
% Creates 4 figures (3-4 only if makeFH1sweeps):
%   1- fixed PRM location and FH1 length, plot pr vs distance from FH2
%       attachment point (with 3 panels: double, dimer, ratio)
%   2- fixed FH1 length, plot pr at attachment point vs. PRM location (with
%       5 panels: double, dimer, ratio, zoomed in dimer, and log10 dimer)
%   3- PRM at NT, plot pr at attachment point vs. FH1 length (with
%       5 panels: double, dimer, ratio, zoomed in dimer, and log10 dimer)
%   4- PRM at 0.5 FH1 length, plot pr at attachment point vs. FH1 length (with
%       5 panels: double, dimer, ratio, zoomed in dimer, and log10 dimer)
arguments
    n1 double
    fh1length double
    ...
    for i=1:length(FH2sizes)
        ...
        fplot(ax_double(1),f_double,[0 40],"DisplayName",num2str(FH2size),"Color",coloroptions(i,:));
